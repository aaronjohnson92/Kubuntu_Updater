...
sudo -H python3 -m pip install pyttsx3 chatterbot PyAudio PocketSphinx SpeechRecognition bleach boto boto3 botocore bz2file cffi click colorama cryptography docutils fire flashtext Flask future gensim html5lib httplib2 idna imbalanced-learn jedi Jinja2 jupyter jupyter-console jupyter-core kappa Keras luminoth lxml nltk notebook numpy pandas pendulum Pillow pyasn1 pycrypto pygobject pymongo PyNaCl PyOpenGL PyQt5 PySDL2 pytz PyYAML requests rsa scikit-learn scipy selenium setuptools sip six SQLAlchemy tensorflow tensorflow-gpu Theano tornado urllib3 wheel xlwt pyexcel pyexcel-io pyexcel-xls pyexcel-xlsx pyexcel-ods3 pyexcel-ods pyexcel-xlsxw pyexcel-xlsxr pyexcel-odsr pyexcel-htmlr pyexcel-text pyexcel-handsontable pyexcel-pygal pyexcel-sortable pyexcel-gantt nose pyglet chipmunk 

cd `pwd`/Libraries 
sudo git clone https://github.com/kivy/cymunk.git
cd cymunk
sudo -H python3 setup.py build_ext install

cd ~
sudo -H python3 -m pip install git+https://github.com/kivy/kivy.git
